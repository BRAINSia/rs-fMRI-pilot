...
from nipype.interfaces.base import (Directory, CommandLineInputSpec, CommandLine, TraitedSpec, traits,
...
class To3DInputSpec(AFNICommandInputSpec):
    prefix = traits.File(argstr='-prefix %s', mandatory=False, genfile=False, hash_file=False,
                         desc='Output file prefix')
    suffix = traits.Str(argstr='%s', mandatory=False, desc='Output file suffix')
    infolder = traits.Either(Directory(), File(), desc='folder with DICOM images to convert', argstr='%s',
                         position=-1, mandatory=True, exists=True)  #TODO: fix description for when File()
    ...
    funcparams = traits.Str(argstr='-time:%s', desc='parameters for functional data')


class To3DOutputSpec(TraitedSpec):
    out_file = File(desc='converted file (.BRIK)')
    # out_file_head = File(desc='.HEAD file')
...
class To3D(AFNICommand):
    ...
    >>> To3D.inputs.infolder = "/paulsen/MRx/FMRI_HD_120/0057/55943/ANONRAW/FMRI_RestingStateConnectivity/13/"  # path.join(basedir, 'data')
    >>> To3D.inputs.filetype = 'anat'
    >>> To3D.inputs.orient = 'RAS'
    >>> To3D.inputs.funcparams = 'zt %s alt+z2'
    >>> res = To3D.run() #doctest: +SKIP
    >>> To3D.cmdline == 'to3d -datum float -anat -time:zt %s alt+z2 -orient RAS /paulsen/MRx/FMRI_HD_120/0057/55943/ANONRAW/FMRI_RestingStateConnectivity/13/.*.IMA'
    True
    >>> To3D.inputs.infolder = '/paulsen/MRx/PHD_024/2727/12082/ANONRAW/FMRI_RestingStateConnectivity/4/'
    >>> To3D.cmdline == 'to3d -datum float -anat -time:zt %s alt+z2 -orient RAS /paulsen/MRx/PHD_024/2727/12082/ANONRAW/FMRI_RestingStateConnectivity/4/*.dcm'
    True
   """
    _cmd = 'to3d'
    ...
    def _format_arg(self, opt, spec, val):
        if opt == 'orient':
            ...
            # Check for .dcm, then .ini, and finally no extenstion
            if glob.glob(os.path.join(self.inputs.infolder, '*.dcm')) != []:
                return os.path.join(self.inputs.infolder, '*.dcm')
            elif glob.glob(os.path.join(self.inputs.infolder, '*.IMA')) != []:
                return os.path.join(self.inputs.infolder, '*.IMA')
            elif glob.glob(os.path.join(self.inputs.infolder, '.*.IMA')) != []:
                return os.path.join(self.inputs.infolder, '.*.IMA')
            elif os.path.isfile(self.inputs.infolder):
                # Case: if input is a file (ANALYZE .hdr, etc.)
                return self.inputs.infolder
            else:
                raise IOError
                #return os.path.join(self.inputs.infolder, '*')
        elif opt == 'prefix':
            pass
    ...
    def _list_outputs(self):
        ...
        brik_filename = self._gen_fname(basename=self.inputs.prefix,
                                              suffix='')
        outputs['out_file'] = brik_filename
        # outputs['out_file_head'] = brik_filename[:-5] + ".HEAD"
        return outputs
...
class Refit(AFNICommand):
    ...
    # TODO: check for existance of .HEAD file!
...
class TStat(AFNICommand):
    ...
    def _list_outputs(self):
        outputs = self.output_spec().get()
        if not isdefined(self.inputs.out_file):
            outputs['out_file'] = self._gen_fname(self.inputs.in_file, suffix=self.inputs.suffix)
        else:
            outputs['out_file'] = os.path.abspath(self.inputs.out_file)
        return outputs
...
class Fim(AFNICommand):
    ...
    def _gen_filename(self, name):
        if name == 'out_file':
            filename, _, __ = self._list_outputs()[name].rpartition('+')
            return filename
        return super(Fim, self)._gen_filename(name)
...
class CalcInputSpec(DynamicTraitedSpec, AFNICommandInputSpec):
    ...
    in_file_b = File(position=1,argstr='-b %s', mandatory=False, exists=True)
